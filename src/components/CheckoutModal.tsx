...
import {
  ...
  KeyRound, Eye, EyeOff, QrCode, Upload,
} from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {step !== 'submitting' && (
          <div className="px-6 pt-6 pb-5 border-b border-yellow-500/10">
            ...
            <h2 className="font-orbitron text-xl font-black text-white">
              {step === 'pending' ? 'Pending Verification' : step === 'failed' ? 'Something Went Wrong' : 'Complete Your Order'}
            </h2>
        ...
        {/* ── STEP 2: PAY VIA QR + SUBMIT ── */}
        {step === 'payment' && (
          <div className="p-6 space-y-5">
            <p className="text-gray-400 font-inter text-sm">Scan the QR code below and pay the exact amount shown. Then submit your order for verification.</p>
            ...
            <div>
              <label className="block font-rajdhani text-xs font-bold uppercase tracking-widest text-yellow-500 mb-2">
                Payment Screenshot <span className="text-gray-600 font-normal normal-case tracking-normal">(recommended)</span>
              </label>
              <label className={`group flex flex-col items-center justify-center gap-2 w-full py-6 rounded-xl border-2 border-dashed cursor-pointer transition-all duration-300 ${fileName ? 'border-yellow-500/60 bg-yellow-500/5' : 'border-yellow-500/25 bg-yellow-500/3 hover:border-yellow-500/50 hover:bg-yellow-500/8'}`}>
                <input type="file" accept="image/*" className="hidden" onChange={handleFile} />
                {fileName ? (
                  <>
                    <Clock className="w-8 h-8 text-yellow-400" />
                    <p className="font-rajdhani text-xs font-bold text-yellow-400 uppercase tracking-wider">Ready — Pending Verification</p>
                    <p className="font-inter text-xs text-gray-500 truncate max-w-[200px]">{fileName}</p>
                  </>
                ) : (
                  <>
                    <Upload className="w-7 h-7 text-yellow-500" />
                    <p className="font-rajdhani text-xs font-bold text-white uppercase tracking-wider">Upload Screenshot After Payment</p>
                    <p className="text-xs text-gray-600 font-inter">PNG, JPG up to 10MB</p>
                  </>
                )}
              </label>
            </div>
            ...
            <div className="p-3 rounded-xl bg-yellow-500/8 border border-yellow-500/25 flex items-center gap-2.5">
              ...
              <p className="font-inter text-xs text-yellow-200 leading-relaxed font-medium">
                A screenshot alone does not confirm your order. Your order will be processed only after our team manually verifies the payment.
              </p>
            </div>

            <button onClick={submitOrder} className="order-btn w-full py-4 rounded-xl font-orbitron font-black uppercase tracking-widest text-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
        ...
        {step === 'pending' && (
          <div className="p-6 flex flex-col items-center text-center space-y-5">
            ...
            <div>
              <h3 className="font-orbitron text-xl font-black text-yellow-400 mb-1">Order Submitted</h3>
            ...
            <div className="w-full p-4 rounded-xl bg-yellow-500/8 border border-yellow-500/20 text-left">
              <p className="font-inter text-sm text-gray-300 leading-relaxed">
                Your order will be processed only after successful payment verification. Send your payment screenshot to us on Instagram or Telegram to speed this up.
              </p>
            </div>
            ...
            <div className="w-full p-4 rounded-xl bg-white/3 border border-white/8 space-y-1.5 text-left">
              ...
              {[
                ...
                <div key={k} className="flex justify-between">
                  ...
                  <span className={`font-inter text-xs font-medium ${k === 'Amount' ? 'text-yellow-500 font-orbitron font-black' : k === 'Game' || k === 'Status' ? 'text-yellow-400' : 'text-white'}`}>{v}</span>
                </div>
              ))}
            </div>
