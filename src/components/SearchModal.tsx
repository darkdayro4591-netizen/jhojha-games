...
import { X, Search, ShoppingCart, Star, Instagram, Send, SlidersHorizontal, Loader2 } from 'lucide-react';
...
export default function SearchModal({ onClose }: Props) {
  ...
  const suggestions = useMemo(() => {
    ...
    return games.filter(g => g.title.toLowerCase().includes(q)).slice(0, 5).map(g => g.title);
  }, [query, games]);
  ...
  const results = useMemo(() => {
    ...
    return games.filter(g => {
      ...
      const matchesFilter =
        activeFilter === 'All' || g.category.toLowerCase() === activeFilter.toLowerCase();
      return matchesQuery && matchesFilter;
    });
  }, [query, activeFilter, games]);
  ...
  const displayed   = isSearching ? results : games;
  const hasResults  = displayed.length > 0;

  return (
    <>
      <div className="fixed inset-0 z-[180] flex flex-col" style={{ background: 'rgba(0,0,0,0.97)', backdropFilter: 'blur(16px)' }}>
        {/* Top bar */}
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="Search games — GTA, Outlast, God of War..."
          ...
          <div className="max-w-4xl mx-auto px-4 py-6">
            ...
            {!loading && isSearching && (
              <div className="flex items-center gap-2 mb-5">
                <span className="font-rajdhani text-sm font-bold uppercase tracking-widest text-gray-500">
                  {hasResults ? `${displayed.length} result${displayed.length !== 1 ? 's' : ''} found` : 'No results'}
                </span>
                {query && (
                  <>
            ...
            {!loading && isSearching && !hasResults && (
              <div className="flex flex-col items-center text-center py-16 px-4">
                ...
                <h3 className="font-orbitron text-xl font-black text-white mb-2">Game Not Found</h3>
                <p className="font-inter text-sm text-gray-400 leading-relaxed max-w-sm mb-2">
                  <span className="text-yellow-400 font-semibold">"{query}"</span> is not currently in our catalog.
                </p>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full max-w-xs">
                  <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl bg-gradient-to-r from-purple-600 via-pink-500 to-yellow-500 text-white font-rajdhani font-bold uppercase tracking-widest text-sm hover:scale-[1.02] transition-all duration-300"
                  >
                    <Instagram className="w-4 h-4" /> Contact on Instagram
                  </a>
                  <a href={TELEGRAM_URL} target="_blank" rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl bg-gradient-to-r from-blue-600 to-sky-500 text-white font-rajdhani font-bold uppercase tracking-widest text-sm hover:scale-[1.02] transition-all duration-300"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {displayed.map((game, idx) => (
                  <div
                    ...
                    style={{ animation: `fadeInUp 0.4s ease ${Math.min(idx, 8) * 0.05}s both` }}
                  ...
                  >
                    <div className="relative h-44 overflow-hidden flex-shrink-0">
                      ...
                      <div className="absolute top-2.5 left-2.5 flex flex-col gap-1.5 z-10">
                        {game.badge && (
                          <span className="badge-new px-2.5 py-0.5 rounded-md font-rajdhani text-xs font-bold uppercase tracking-wider">{game.badge}</span>
                        )}
                        {game.discount > 0 && (
                          <span className="badge-sale px-2.5 py-0.5 rounded-md font-rajdhani text-xs font-bold uppercase tracking-wider">-{game.discount}%</span>
                        )}
                      </div>
                      ...
                      <div className="absolute bottom-2.5 right-2.5 flex items-center gap-1 px-2 py-0.5 rounded-md bg-black/70 border border-yellow-500/20">
                        ...
                        <span className="text-[10px] font-rajdhani font-bold text-yellow-500">{game.rating}.0</span>
                      </div>
                    </div>
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        {game.originalPrice > game.salePrice && (
                          <>
                            <span className="text-xs text-gray-600 line-through font-inter">₹{game.originalPrice.toLocaleString('en-IN')}</span>
                            <span className="text-[10px] font-bold text-green-400 bg-green-400/10 px-1.5 py-0.5 rounded">{game.discount}% OFF</span>
                          </>
                        )}
                      </div>
                      ...
                      <button
                        onClick={() => game.inStock && setCheckout({ title: game.title, price: game.salePrice })}
                        disabled={!game.inStock}
                        className={`order-btn mt-auto w-full py-2.5 rounded-xl font-rajdhani text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-2 transition-transform duration-200 ${game.inStock ? 'hover:scale-[1.02]' : 'opacity-40 cursor-not-allowed'}`}
                      >
